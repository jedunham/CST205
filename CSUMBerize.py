...
def CSUMBBlend(pic):  
  ...
  goldenSand = makeColor(132, 114, 72)
  valleyGreen = makeColor(0, 120, 86)
  amount = 0.5
  for x in range(0, getWidth(pic)/3):
    for y in range(0, getHeight(pic)):
      ...
      newBBGreen = 47*amount + getGreen(pixel)*(1-amount)
      newBBBlue = 93*amount + getBlue(pixel)*(1-amount)
  ...
  for x in range(getWidth(pic)/3, getWidth(pic)*2/3):
    for y in range(0, getHeight(pic)):
      pixel = getPixel(pic, x, y)
      px = getColor(pixel)
      newGSRed = 132*amount + getRed(pixel)*(1-amount)
      newGSGreen = 114*amount + getGreen(pixel)*(1-amount)
      newGSBlue = 72*amount + getBlue(pixel)*(1-amount)
  ...
  for x in range(getWidth(pic)*2/3, getWidth(pic)):
    for y in range(0, getHeight(pic)):
      ...
      newVGGreen = 120*amount + getGreen(pixel)*(1-amount)
      newVGBlue = 86*amount + getBlue(pixel)*(1-amount)
